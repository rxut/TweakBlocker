...
function xxGetProperties()
{
    ...
    for (zzP = Level.PawnList; zzP != none; zzP = zzP.nextPawn)
    {
        ...
        if (zzP.IsA('PlayerPawn') && !zzP.IsA('Spectator'))
            zzProps.xxAddProperties(PlayerPawn(zzP),zzP.texture,zzP.Mesh,zzP.bMeshEnviroMap,zzP.bUnlit,zzHasBelt,zzP.LightRadius,zzP.DrawScale);
    }
}
...
simulated function xxCheck(int zzKey, TBActor zzA, TBSettings zzS, TBPlayerDisplayProperties zzProps)
{
    ...
    // Check for unlit pickupg tweaks
    if (zzA.bCheckPickupUnlit)
    {
        foreach Level.AllActors(class'Pickup', zzPickup)
        {
            zzReply = xxGetTextureProperties(zzPickup);

            if (xxGetToken(zzReply, 5) == "1")
                {
                    if (zzPickup.bUnlit == True)
                        {
                        xxAddTweak(zzTweaksReply,"Bright Pickup Tweak (Unlit)");
                        }
                }
        }
        zzTestsExecuted++;
    ...
    // CheckLODBias ~ Check the LODBias value for all PlayerPawns
   if (zzA.bCheckLODBias)
    {
         foreach Level.AllActors(class'PlayerPawn', zzPP)
            {   
                zzPP.ConsoleCommand("set UMenu.UMenuMenuBar ModMenuClass TweakBlocker.TBModMenu");
                lodBias = zzA.GetLODBias(zzPP);

                    if (float(lodBias) >= 4.0) // Convert lodBias to float before comparing
                    {
                        xxAddTweak(zzTweaksReply, "LODBias Too high");
                    }
            }
    ...
    if (zzA.bCheckPlayerSkins)
    {
        ...
        foreach Level.AllActors(class'PlayerPawn',zzPP)
        {
            ...
            Log("zzReply bCheckPlayerSkins: " @zzReply);
            Log("xxGetToken(zzReply, 1) bCheckPlayerSkins: " @xxGetToken(zzReply, 1));
            // Player Unlit Check
            if(zzA.bCheckPlayerUnlitSkins)
            {
...
simulated function xxCheckReply (string zzString)
{
    ...
    zzTestsExpected = (int(zzActor.bCheckRMode) + int(zzActor.bCheckRendering) +
        int(zzActor.bCheckPlayerSkins) + int (zzActor.bCheckFlags) + int(zzActor.bCheckPickupUnlit) + int(zzActor.bCheckLODBias) + int(zzActor.bCheckWeaponModels));
...
simulated function string xxGetTextureProperties(Object zzO)
{
    ...
    if (zzO.IsA('Actor'))
    {
        ...
        zzHidden = string(int(zzA.bHidden));
        return string(zzA.Texture)$chr(9)$int(zzA.default.bUnlit)$chr(9)$zzTexUnlit$chr(9)$zzTexTrans$chr(9)$zzTexInvis$chr(9)$int(zzA.bMeshEnviroMap)$chr(9)$zzHidden; // Added bHidden
    }
    else if (class<Actor>(zzO) != none)
    {
        ...
        zzHidden = string(int(zzC.default.bHidden));
        return string(zzC.default.Texture)$chr(9)$int(zzC.default.bUnlit)$chr(9)$zzTexUnlit$chr(9)$zzTexTrans$chr(9)$zzTexInvis$chr(9)$int(zzC.default.bMeshEnviroMap)$chr(9)$zzHidden; // Added bHidden
    }
    else
    {
